...
class Encoder(nn.Module):
    ...
    def setup(self):
        ...
        for i in range(self.n_layers):
            ...
            norm_layers_1.append(nn.LayerNorm(epsilon=1e-5))
            ffn_layers.append(
                FFN(
                    self.hidden_channels,
                    self.hidden_channels,
            ...
            norm_layers_2.append(nn.LayerNorm(epsilon=1e-5))
    ...
    def __call__(self, x, x_mask,train=True):
        attn_mask = jnp.expand_dims((jnp.expand_dims(x_mask,1) * jnp.expand_dims(x_mask,-1)),1).squeeze(axis=-1)
        x = x * x_mask
        for i in range(self.n_layers):
            y = self.attn_layers[i](x, x, attn_mask=attn_mask,deterministic=not train)
            x = self.norm_layers_1[i](x + y)
            y = self.ffn_layers[i](x, x_mask,deterministic=not train)
